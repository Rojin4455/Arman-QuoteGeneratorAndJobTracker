...
def _bootstrap_location(location_id, access_token):
    ...
    try:
        ...
        logger.exception("GHL OAuth tokens(): referral custom fields ensure failed location_id=%s", location_id)
...
def _handle_install_webhook(data):
    ...
    if GHLAuthCredentials.objects.filter(location_id=location_id).exists():
        ...
        if company_level_oauth:
            ...
            for loc in locations:
                ...
                try:
                    ...
                    try:
                        from referral_app.ghl_sync import ensure_referral_invite_custom_fields
                        ensure_referral_invite_custom_fields(credentials)
                    except Exception:
                        logger.exception(
                            "GHL OAuth tokens(): referral custom fields ensure failed location_id=%s",
                            location_id,
                        )
